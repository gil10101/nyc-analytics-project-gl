...
-- Only keep categories that actually appear in our data
used_categories AS (
...
age_category_dimension AS (
   SELECT
       {{ dbt_utils.generate_surrogate_key(['cd.age_category_name']) }} AS age_category_key,
   ...
   INNER JOIN used_categories uc ON cd.age_category_name = uc.age_category_name
)
